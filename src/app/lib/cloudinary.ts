...
export const imageFileFilter: multer.Options['fileFilter'] = (_req, file, callback) => {
  const allowedImageTypes: readonly string[] = FILE_UPLOAD.ALLOWED_IMAGE_MIME_TYPES;

  if (!allowedImageTypes.includes(file.mimetype)) {
    return callback(new AppError(status.BAD_REQUEST, 'Only JPEG, PNG, or WEBP images are allowed'));
  }

  callback(null, true);
};

export const postMediaFileFilter: multer.Options['fileFilter'] = (_req, file, callback) => {
  const allowedPostMediaTypes: readonly string[] = FILE_UPLOAD.ALLOWED_POST_MEDIA_MIME_TYPES;

  if (!allowedPostMediaTypes.includes(file.mimetype)) {
    return callback(new AppError(status.BAD_REQUEST, 'Only image or PDF files are allowed'));
  }
...
export const profileAvatarStorage = new CloudinaryStorage({
  ...
  params: async (req: Request, file: Express.Multer.File) => {
    return {
      ...
      resource_type: 'image',
      public_id: createSafePublicId(`avatar-${req.user?.id ?? 'user'}`, file.originalname),
    };
  },
});
...
export const profileCoverStorage = new CloudinaryStorage({
  ...
  params: async (req: Request, file: Express.Multer.File) => {
    return {
      ...
      resource_type: 'image',
      public_id: createSafePublicId(`cover-${req.user?.id ?? 'user'}`, file.originalname),
    };
  },
});

export const taskAttachmentStorage = postMediaStorage;
export const userProfileStorage = profileAvatarStorage;
